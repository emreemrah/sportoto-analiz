...
## YAPILDI — Radar 5 "Oran" modu: sağda SON GÜNÜN oranı (2026-08-08)

Kullanıcı isteği: *"oynanma yerine basınca sağ tarafta oynanma yüzdeleri ve
buna göre maçlar filtrelensin, orana basınca sağ tarafta son günün oranları
yazsın ve orana göre maçlar gelsin."*

- [x] "Oran" modunda satırın sağındaki özet, oynanma yüzdesi yerine **son günün
      1/X/2 oranını** gösteriyor (`Pzt · 1 1.61 · X 3.20 · 2 4.25`).
- [x] Gün seçimi oynanma tarafındaki mantığın aynısı: **gerçek kaydı olan son
      geçmiş/bugün günü**; gelecek gün asla.
- [x] "Oynanma %" ve hafta pencereleri (Tüm/5/10/15 Hafta) **aynen** eskisi gibi.
- [x] Saf mantık ekranın dışında: `app/src/radar/oranOzeti.js`.
- [x] Yeni istek/uç YOK: Radar 4'ün `daily-odds` ucu ve aynı çekme kilidi
      (`cekilenOran`) kullanılıyor; yalnız mod seçilince çekiliyor.

### Yapılamayan — orana göre SÜZME (dürüstlük notu)
Geçmiş maçların oranı arşivde YOK: `/api/radar/position-matches` yalnız
`played.pct` taşıyor. Bu yüzden "orana göre maçlar gelsin" bugünkü veriyle
karşılanamaz. Sahte eşleşme/tahmini oran ÜRETİLMEDİ; ekran bunu açıkça yazıyor:
*"Geçmiş maçların oran kaydı arşivde yok — bu süzgeç veri biriktikçe çalışacak."*
Sıradaki iş, günlük oran mührünün geçmiş maç arşivine (position-matches) da
yazılması; ondan sonra süzme tek satırlık bir değişiklikle açılır.

### Dosyalar
- YENİ `app/src/radar/oranOzeti.js` (gün seçimi + oran geçerlilik kuralı)
- `app/src/screens/RadarScreen.js` (sağ özet moda göre; oran ucu Radar 5'te de)
- `app/src/components/RadarTabHeaders.js` (mod ipucu: sağda ne yazdığı)
- YENİ `app/test/oran-ozeti.test.mjs` (7) · `app/test-ui/radar-ekrani.test.jsx` (+3)

### Doğrulama
- app node testleri: **523 test, 522 geçti** (`simple-test.mjs` bu işten ÖNCE de kırık).
- jest UI (Radar 5 süzgeç bloğu): 10 testin 9'u geçti; kalan 1 (`yeni modlar
  dönem satırında…`) **HEAD kopyasında da** aynı şekilde düşüyor — ilk render
  5 sn'lik jest sınırını aşıyor (soğuk başlangıç), bu işten bağımsız.
- Bozarak doğrulama (lessons.md §5): `oranUclusu` naive `Number()`e çevrildi →
  (b) ve (c) kırmızı; `oranGunu`daki `!g.future` süzgeci kaldırıldı → (d)
  kırmızı. İkisi de geri alındı, 7/7 yeşil.
