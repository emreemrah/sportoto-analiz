...
test('2. Geri çekilme SEVİYE 1: sağlayıcı+sıra+bant+hareket (örneklem yeterliyse)', () => {
  const records = [
    ...
    ...Array.from({ length: 7 }, () => rec('k3', 14, '1', 72, 60)),
    ...Array.from({ length: 3 }, () => rec('k3', 14, 'X', 71, 62)),
    ...Array.from({ length: 2 }, () => rec('k3', 14, '2', 73, 61)),
    // gürültü: farklı sıra/sağlayıcı
    ...Array.from({ length: 20 }, () => rec('nesine', 14, '2', 72, 60)),
    ...Array.from({ length: 20 }, () => rec('k3', 3, '2', 72, 60)),
  ];
  ...
  assert.equal(r.sample, 12, 'yalnız k3 + 14. sıra + bant + hareket');
...
test('3. Geri çekilme SEVİYE 2→3: sıra örneklemi yetmezse segment, o da yetmezse bant', () => {
  ...
  const records2 = [
    ...Array.from({ length: 5 }, () => rec('k3', 14, '1', 72)),
    ...Array.from({ length: 6 }, () => rec('k3', 2, '1', 71)),
  ];
...
test('4. n<10 → YÖN SİNYALİ YOK (dürüst "sistem öğreniyor")', () => {
  const records = Array.from({ length: 6 }, () => rec('k3', 14, '1', 72, 60));
  const r = findSimilarDna(records, { provider: 'k3', position: 14, favoriteSymbol: '1', closeValue: 72, moveValue: 10 });
...
test('5. Sağlayıcılar HAM veride karışmaz: sorgu yalnız kendi sağlayıcısını görür', () => {
  const records = [
    ...
    ...Array.from({ length: 12 }, () => rec('k3', 14, '1', 72, 60)), // k3'te hep 1
  ];
  const r = findSimilarDna(records, { provider: 'k3', position: 14, favoriteSymbol: '1', closeValue: 72, moveValue: 10 });
  assert.equal(r.sample, 12, 'nesine kayıtları k3 sorgusuna KARIŞMADI');
...
test('6. Sağlayıcı uzlaşması: medyan + fark + uzlaşma/çatışma bayrağı', () => {
  ...
  const single = providerConsensus({ k3: { '1': 60, X: 25, '2': 15 } });
