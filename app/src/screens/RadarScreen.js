...
export default function RadarScreen({ navigation }) {
  ...
  // dnaKey: "<maçNo>|<kaynak>" · dnaWeeks: null | 5 | 10 | 15
  const [dnaKey, setDnaKey] = useState(null);
  const [dnaWeeks, setDnaWeeks] = useState(null);
  const [dnaTol, setDnaTol] = useState(2);   // yakınlık: 0 birebir · 1 · 2 · 3
  ...
  // OYNANMA DNA'SI — açık kaynak satırı veya hafta filtresi değişince çekilir.
  // Gün değişirse panel kapanır (aşağıda), çünkü DNA seçili güne bağlıdır.
  useEffect(() => {
    if (!dnaKey) { setDnaData(null); dnaSorguRef.current = null; return undefined; }
    const [noStr, source] = String(dnaKey).split('|');
    const rid = view?.roundId ?? selectedId;
    if (rid == null || !source) return undefined;
    ...
    const sorgu = `${rid}|${dnaKey}|${playedDay}|${dnaWeeks}|${dnaTol}`;
    const sessiz = dnaSorguRef.current === sorgu;
    dnaSorguRef.current = sorgu;
    let iptal = false;
    if (!sessiz) setDnaBusy(true);
    api.radarPlayedDna(rid, Number(noStr), source, playedDay, dnaWeeks, dnaTol)
      .then((d) => { if (!iptal) setDnaData(d); })
      .catch(() => { if (!iptal && !sessiz) setDnaData(null); })
      .finally(() => { if (!iptal) setDnaBusy(false); });
    return () => { iptal = true; };
  }, [dnaKey, dnaWeeks, dnaTol, playedDay, selectedId, view, playedTick]);
  ...
  return (
    ...
    <View style={[styles.container, { backgroundColor: 'transparent' }]}>
      ...
      <FlatList
        data={listData}
        extraData={[tab, legacyView, filter, legacyFilter, sortMode, expandedNo, picks, dnaPeriod, positionDna, dailyOdds, oddsDay, dailyPlayed, playedDay, dnaKey, dnaWeeks, dnaTol, dnaData, dnaBusy]}
