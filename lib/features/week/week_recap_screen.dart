...
const Map<String, ({String icon, String title, Color color})> _hl = {
  ...
  'system-win': (
    icon: '🤖',
    title: 'Sistem bildi, sen bilemedin',
...
class _WeekRecapScreenState extends State<WeekRecapScreen> {
  ...
  Future<void> _boot() async {
    ...
    try {
      final r = (await api.rounds() as Map).cast<String, dynamic>();
      if (!mounted) return;
      setState(() => _rounds = r);
  ...
  Widget build(BuildContext context) {
    if (_loading && _rounds == null) {
      return _kabuk(const LoadingState(message: 'Hafta kapanışı hazırlanıyor…'));
    }
    if (_error != null) {
      return _kabuk(
    ...
    final curIdx =
        all.indexWhere((r) => (r as Map)['id'] == _rounds?['currentRoundId']);
  ...
  Widget _senVsSistem(WeekRecap recap) {
    final u = recap.user;
    final sys = recap.system;
    final winner = recap.head2head?.winner;
    return Padding(
      padding: const EdgeInsets.only(bottom: Spacing.md),
        ...
        child: Row(
          ...
          children: [
            ...
            Expanded(
              child: _skorSutunu(
                baslik: 'SİSTEM',
                deger: sys != null ? '${sys.correct}/${sys.made}' : '—',
                alt: sys != null ? '%${sys.accuracy} isabet' : 'tahmin yok',
                onde: winner == 'system',
                ton: AppColors.info,
              ),
            ),
          ...
          Padding(
            ...
            child: Wrap(
              ...
              children: [
                ...
                if (r.system case final s?) _cip('Sistem ', s.pick, ''),
              ],
            ),
          ),
        ],
              ...
              Builder(
                builder: (_) {
                  ...
                  final metin = Text(
                    ...
                    style: TextStyle(
                      color: h.c ??
                          (baslik ? AppColors.textMuted : AppColors.text),
  ...
  Widget _bosKart({String? icon, required String baslik, required String metin}) =>
      Container(
        margin: const EdgeInsets.only(bottom: Spacing.md),
